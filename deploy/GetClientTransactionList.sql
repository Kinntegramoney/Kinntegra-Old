CREATE procedure [dbo].[GetClientTransactionList]
	@AssociateId bigint,
	@FromDate date,
	@ToDate date,
			...
			from ClientTransaction ct inner join TransactionType tt on ct.TransactionTypeId = tt.Id
				...
				inner join (
					...
					where ct1.TradeStatus not in ('','In-Complete')
						and (convert(date,DATEADD(MINUTE, 330, TransactionDate)) between @FromDate and @ToDate)
						and tt1.Code = 'B'
						and tp1.Code = 'L'
					group by ct1.Id
					...
					where ct1.TradeStatus not in ('','In-Complete')
						...
						and ct1.Id not in (
							...
							from ClientTransaction ct1 inner join ClientTransactionPortfolio ctp1 on ct1.Id = ctp1.ClientTransactionId
								...
								inner join TransactionPortfolioType tpt1 on ctpt1.TransactionPortfolioTypeId = tpt1.Id
							where ct1.TradeStatus not in ('','In-Complete')
								and (convert(date,DATEADD(MINUTE, 330, TransactionDate)) between '2026-05-15' and '2026-05-15')
								and tt1.Code = 'S'
								and tp1.Code = 'L'
								and tpt1.Code = 'A'
								and Amount > 0
