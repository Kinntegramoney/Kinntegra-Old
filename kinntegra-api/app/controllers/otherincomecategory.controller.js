...
exports.GetIncomeCategoryList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetIncomeCategory");
...
exports.GetIncomeCategoryById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetIncomeCategoryById");
        if (readResult.recordset.length > 0) {
...
exports.SaveIncomeCategory = async (req, res) => {
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            const result = await request.execute("InsertIncomeCategory");
            ...
            const result = await request.execute("UpdateIncomeCategory");
        }
        ...
        //     TransactionId: Id,
        //     TransactionType: 'IncomeCategory',
...
exports.DeleteIncomeCategory = async (req, res) => {
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteIncomeCategory");
        await transaction.commit();
