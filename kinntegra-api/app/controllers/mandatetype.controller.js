...
exports.GetMandateTypeList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetMandateTypes");
...
exports.GetMandateTypeById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetMandateTypeById");
        if (readResult.recordset.length > 0) {
...
exports.SaveMandateType = async (req, res) => {
    var { Id, Name,Code } = req.body;

    var errorMessage = "Error while saving Mandate Type data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                    .input("Name", Name)
                    .input("Code", Code)
            const result = await request.execute("InsertMandateType");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                    .input("Name", Name)
                    .input("Code", Code)
            const result = await request.execute("UpdateMandateType");
        }
        ...
        //     TransactionId: Id,
        //     TransactionType: 'MandateType',
        //     Status: TransactionMode,
        //     Description: ''
        // };

        // var activityId = await activityLogController.CreateActivityLog(req, res, activityData);
        res.status(200).send({ Status: true, Message: "Mandate Type Saved Successfully.", Data: { Id: Id } });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.DeleteMandateType = async (req, res) => {
    var { Id } = req.body;


    var errorMessage = "Error while deleting Mandate Type data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteMandateType");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Mandate Type Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
