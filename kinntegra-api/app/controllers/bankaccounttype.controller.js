...
exports.GetBankAccountTypeList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetBankAccountTypes");
...
exports.GetBankAccountTypeById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetBankAccountTypeById");
        if (readResult.recordset.length > 0) {
...
exports.SaveBankAccountType = async (req, res) => {
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                    .input("Name", Name)
                    .input("Code", Code)
            const result = await request.execute("InsertBankAccountType");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                    .input("Name", Name)
                    .input("Code", Code)
            const result = await request.execute("UpdateBankAccountType");
        }
...
exports.DeleteBankAccountType = async (req, res) => {
    var { Id } = req.body;
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteBankAccountType");
        await transaction.commit();
