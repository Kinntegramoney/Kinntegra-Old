...
exports.GetAddressTypeList = async (req, res) => {
    try {
        ...
        const readResult = await readRequest.execute("GetAddressTypes");
...
exports.GetAddressTypeById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetAddressTypeById");
        if (readResult.recordset.length > 0) {
...
exports.SaveAddressType = async (req, res) => {
    var { Id, Name } = req.body;

    var errorMessage = "Error while saving address type data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                .input("Name", Name)
            const result = await request.execute("InsertAddressType");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                .input("Name", Name)
            const result = await request.execute("UpdateAddressType");
        }
        ...
        //     TransactionId: Id,
        //     TransactionType: 'AddressType',
        //     Status: TransactionMode,
        //     Description: ''
        // };

        // var activityId = await activityLogController.CreateActivityLog(req, res, activityData);
        res.status(200).send({ Status: true, Message: "Address Type Saved Successfully.", Data: { Id: Id } });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.DeleteAddressType = async (req, res) => {
    ...
    var errorMessage = "Error while deleting address type data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteAddressType");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Address Type Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
