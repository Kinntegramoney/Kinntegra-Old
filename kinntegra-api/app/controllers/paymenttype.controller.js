...
exports.GetPaymentTypeList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetPaymentType");
...
exports.GetPaymentTypeById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetPaymentTypeById");
        if (readResult.recordset.length > 0) {
...
exports.SavePaymentType = async (req, res) => {
    ...
    var errorMessage = "Error while saving Payment type data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            const result = await request.execute("InsertPaymentType");
            ...
            const result = await request.execute("UpdatePaymentType");
        }
        ...
        res.status(200).send({ Status: true, Message: "Payment Type Saved Successfully.", Data: { Id: Id } });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.DeletePaymentType = async (req, res) => {
    ...
    var errorMessage = "Error while deleting Payment type data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeletePaymentType");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Payment Type Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
