...
exports.GetTaxSlabList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetTaxSlabs");
...
exports.GetTaxSlabById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetTaxSlabById");
        if (readResult.recordset.length > 0) {
...
exports.SaveTaxSlab = async (req, res) => {
    var { Id, Name, Code, Type } = req.body;

    var errorMessage = "Error while saving tax slab data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                ...
                .input("Code", Code)
                .input("Type", Type)
            const result = await request.execute("InsertTaxSlab");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                ...
                .input("Code", Code)
                .input("Type", Type)
            const result = await request.execute("UpdateTaxSlab");
        }
...
exports.DeleteTaxSlab = async (req, res) => {
    ...
    var errorMessage = "Error while deleting tax slab data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteTaxSlab");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Tax Slab Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.GetTaxSlabListByType = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetTaxSlabByType");
        if (readResult.recordset.length > 0) {
            dataList = readResult.recordset;
        }
