...
exports.GetKycStatus = async (req, res) => {
    try {
        ...
        const readResult = await readRequest.execute("GetKYCStatuses");
...
exports.GetKycStatusById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetKYCStatusById");
        if (readResult.recordset.length > 0) {
...
exports.SaveKycStatus = async (req, res) => {
    var { Id, Name,Code } = req.body;

    var errorMessage = "Error while saving Kyc Status data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                    .input("Name", Name)
                    .input("Code", Code)
            const result = await request.execute("InsertKYCStatus");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                    .input("Name", Name)
                    .input("Code", Code)
            const result = await request.execute("UpdateKYCStatus");
        }
        ...
        //     TransactionId: Id,
        //     TransactionType: 'KycStatus',
        //     Status: TransactionMode,
        //     Description: ''
        // };

        // var activityId = await activityLogController.CreateActivityLog(req, res, activityData);
        res.status(200).send({ Status: true, Message: "Kyc Status Saved Successfully.", Data: { Id: Id } });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.DeleteKycStatus = async (req, res) => {
    var { Id } = req.body;


    var errorMessage = "Error while deleting Kyc Status data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteKYCStatus");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Kyc Status Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
