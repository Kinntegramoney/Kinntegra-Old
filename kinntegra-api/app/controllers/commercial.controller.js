...
exports.GetCommercialList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetCommercial");
        ...
        if (dataList.length > 0) {
            for (let i = 0; i < dataList.length; i++) {
                let dataItem = {
                    ...
                    FieldName: dataList[i].FieldName,
...
exports.GetCommercialById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetCommercialById");
        if (readResult.recordset.length > 0) {
...
exports.SaveCommercial = async (req, res) => {
    var { Id, Name,FieldName } = req.body;
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            const result = await request.execute("InsertCommercial");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                    .input("Name", Name)
                    .input("FieldName", FieldName)
            const result = await request.execute("UpdateCommercial");
        }
...
exports.DeleteCommercial = async (req, res) => {
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteCommercial");
        await transaction.commit();
