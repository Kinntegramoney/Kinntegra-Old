...
exports.GetTaxStatusList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetTaxStatus");
...
exports.GetTaxStatusById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetTaxStatusById");
        if (readResult.recordset.length > 0) {
...
exports.SaveTaxStatus = async (req, res) => {
    var { Id, Name, Code, Type } = req.body;

    var errorMessage = "Error while saving tax status data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                ...
                .input("Code", Code)
                .input("Type", Type)
            const result = await request.execute("InsertTaxStatus");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                ...
                .input("Code", Code)
                .input("Type", Type)
            const result = await request.execute("UpdateTaxStatus");
        }
        ...
        //     TransactionId: Id,
        //     TransactionType: 'TaxStatus',
        //     Status: TransactionMode,
        //     Description: ''
        // };

        // var activityId = await activityLogController.CreateActivityLog(req, res, activityData);
        res.status(200).send({ Status: true, Message: "Tax Status Saved Successfully.", Data: { Id: Id } });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.DeleteTaxStatus = async (req, res) => {
    var { Id } = req.body;


    var errorMessage = "Error while deleting tax status data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteTaxStatus");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Tax Status Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.GetTaxStatusListByType = async (req, res) => {
    ...
    try {
        ...
        const readRequest = req.app.locals.db.request();
        readRequest.input("Type", Type);
        const readResult = await readRequest.execute("GetTaxStatusByType");
        if (readResult.recordset.length > 0) {
            dataList = readResult.recordset;
        }
