...
exports.GetGrossAnnualIncomeList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetGrossAnnualIncomes");
...
exports.GetGrossAnnualIncomeById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetGrossAnnualIncomeById");
        if (readResult.recordset.length > 0) {
...
exports.SaveGrossAnnualIncome = async (req, res) => {
    var { Id, Name, Code } = req.body;

    var errorMessage = "Error while saving gross annual income data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                .input("Name", Name)
                .input("Code", Code)
            const result = await request.execute("InsertGrossAnnualIncome");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                .input("Name", Name)
                .input("Code", Code)
            const result = await request.execute("UpdateGrossAnnualIncome");
        }
        ...
        //     TransactionId: Id,
        //     TransactionType: 'GrossAnnualIncome',
        //     Status: TransactionMode,
        //     Description: ''
        // };

        // var activityId = await activityLogController.CreateActivityLog(req, res, activityData);
        res.status(200).send({ Status: true, Message: "Gross Annual Income Saved Successfully.", Data: { Id: Id } });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.DeleteGrossAnnualIncome = async (req, res) => {
    var { Id } = req.body;


    var errorMessage = "Error while deleting gross annual income data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteGrossAnnualIncome");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Gross Annual Income Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
