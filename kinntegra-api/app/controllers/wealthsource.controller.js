...
exports.GetWealthSourceList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetWealthSources");
...
exports.GetWealthSourceById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetWealthSourceById");
        if (readResult.recordset.length > 0) {
...
exports.SaveWealthSource = async (req, res) => {
    var { Id, Name, Code } = req.body;

    var errorMessage = "Error while saving wealth source data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                .input("Name", Name)
                .input("Code", Code)
            const result = await request.execute("InsertWealthSource");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                .input("Name", Name)
                .input("Code", Code)
            const result = await request.execute("UpdateWealthSource");
        }
        ...
        //     TransactionId: Id,
        //     TransactionType: 'WealthSource',
        //     Status: TransactionMode,
        //     Description: ''
        // };

        // var activityId = await activityLogController.CreateActivityLog(req, res, activityData);
        res.status(200).send({ Status: true, Message: "Wealth Source Saved Successfully.", Data: { Id: Id } });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.DeleteWealthSource = async (req, res) => {
    var { Id } = req.body;


    var errorMessage = "Error while deleting wealth source data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteWealthSource");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Wealth Source Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
