...
exports.GetOccupationList = async (req, res) => {
    try {
        var dataList = [];

        const readRequest = req.app.locals.db.request();
        const readResult = await readRequest.execute("GetOccupations");
...
exports.GetOccupationById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetOccupationById");
        if (readResult.recordset.length > 0) {
...
exports.SaveOccupation = async (req, res) => {
    var { Id, Name, Code, Type } = req.body;

    var errorMessage = "Error while saving occupation data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                ...
                .input("Code", Code)
                .input("Type", Type)
            const result = await request.execute("InsertOccupation");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                ...
                .input("Code", Code)
                .input("Type", Type)
            const result = await request.execute("UpdateOccupation");
        }
        ...
        //     TransactionId: Id,
        //     TransactionType: 'Occupation',
        //     Status: TransactionMode,
        //     Description: ''
        // };

        // var activityId = await activityLogController.CreateActivityLog(req, res, activityData);
        res.status(200).send({ Status: true, Message: "Occupation Saved Successfully.", Data: { Id: Id } });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.DeleteOccupation = async (req, res) => {
    ...
    var errorMessage = "Error while deleting occupation data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteOccupation");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Occupation Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
