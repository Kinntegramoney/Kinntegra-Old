...
exports.GetAmcList = async (req, res) => {
    try {
        ...
        const readResult = await readRequest.execute("GetAmc");
        ...
        if (dataList.length > 0) {
            for (let i = 0; i < dataList.length; i++) {
                let dataItem = {
                    ...
                    IsIGSTApplicable: dataList[i].IsIGSTApplicable,
...
exports.GetAmcById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetAmcById");
        if (readResult.recordset.length > 0) {
        ...
        if (dataItem != null) {
            data = {
                ...
                IsIGSTApplicable:dataItem.IsIGSTApplicable,
...
exports.SaveAmc = async (req, res) => {
    var { Id, Name,BseCode } = req.body;
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                    ...
                    .input("IsIGSTApplicable", IsIGSTApplicable)
            const result = await request.execute("InsertAmc");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                    ...
                    .input("BseCode", BseCode)
                    .input("IsIGSTApplicable", IsIGSTApplicable)
            const result = await request.execute("UpdateAmc");
        }
...
exports.DeleteAmc = async (req, res) => {
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteAmc");
        await transaction.commit();
