...
exports.GetCountryList = async (req, res) => {
    try {
        ...
        const readResult = await readRequest.execute("GetCountries");
        ...
        if (dataList.length > 0) {
            for (let i = 0; i < dataList.length; i++) {
                let dataItem = {
                    ...
                    FATCACode: dataList[i].FATCACode,
...
exports.GetCountryById = async (req, res) => {
    ...
    try {
        ...
        const readResult = await readRequest.execute("GetCountryById");
        if (readResult.recordset.length > 0) {
...
exports.SaveCountry = async (req, res) => {
    var { Id, Name, Code, FATCACode } = req.body;

    var errorMessage = "Error while saving country data...";
    ...
    try {
        ...
        if (cryptoEngine.ParamDecrypt(Id, true) == 0) {
            ...
            request.output("Id", msSql.BigInt)
                .input("Name", Name)
                .input("Code", Code)
                .input("FATCACode", FATCACode)
            const result = await request.execute("InsertCountry");
            ...
            request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
                ...
                .input("Code", Code)
                .input("FATCACode", FATCACode)
            const result = await request.execute("UpdateCountry");
        }
        ...
        res.status(200).send({ Status: true, Message: "Country Saved Successfully.", Data: { Id: Id } });
    }
    catch (err) {
        console.log(err);
        try {
...
exports.DeleteCountry = async (req, res) => {
    ...
    var errorMessage = "Error while deleting country data...";
    ...
    try {
        ...
        request.input("Id", cryptoEngine.ParamDecrypt(Id, true))
        const result = await request.execute("DeleteCountry");
        await transaction.commit();
        ...
        res.status(200).send({ Status: true, Message: "Country Deleted Successfully.", Data: {} });
    }
    catch (err) {
        console.log(err);
        try {
