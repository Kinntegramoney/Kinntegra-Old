...
@Component({
  selector: 'app-transaction-payment-sip',
  standalone: true,
  ...
  templateUrl: './transaction-payment-sip.component.html',
  styleUrl: './transaction-payment-sip.component.scss',
  providers: [
...
export class TransactionPaymentSipComponent {
  minDate: any;
  maxDate: any;
  ...
  isFirstOrderToday: boolean = false;
  ...
  getClientTransactionDetails() {
    this.transactionService.GetClientTransactionDetails(this.clientTransactionId).subscribe((result) => {
      if (result.Status == true) {
        ...
        var firstOrderData = this.clientTransactionDetails.ClientTransactionPortfolios.filter((x: { IsSIPFirstOrderToday: boolean; }) => x.IsSIPFirstOrderToday == true);

        this.isFirstOrderToday = (firstOrderData.length > 0);
        ...
        for (let i = 0; i < this.clientTransactionDetails.ClientTransactionPortfolios.length; i++) {
          ...
          if (this.clientTransactionDetails.TransactionTypeCode == 'B') {
            this.totalChequePaymentAmount += this.clientTransactionDetails.ClientTransactionPortfolios[i].SIPAmount;
          }
  ...
  onPortfolioEditClicked(portfolioItem: any) {
    ...
    this.router.navigate(['transaction/buy/sip/' + this.clientTransactionId + '/' + portfolioItem.ClientTransactionPortfolioTypeId]);
  }
  ...
  validate(): boolean {
    ...
    if (this.isFirstOrderToday == true) {
  ...
  onProceed() {
    ...
    inputData.append('IsFirstOrderToday', this.isFirstOrderToday.toString());
    if (this.objSinglePayment.InvestmentChequeFile != null) {
    ...
    this.transactionService.SaveClientTransactionSIPPayment(inputData).subscribe(
      (result) => {
        if (result.Status == true) {
          this.isBusy = false;
          var userMessage = "Trade Initiated for Client approval! Do you want to make another transaction?";
          if (this.clientTransactionDetails.TransactionTypeCode == 'B' && this.clientTransactionDetails.TransactionPlanCode == 'SIP') {
            var customTrades = this.clientTransactionDetails.ClientTransactionPortfolios.filter((x: { SIPAllocationType: string; }) => x.SIPAllocationType == 'C');
