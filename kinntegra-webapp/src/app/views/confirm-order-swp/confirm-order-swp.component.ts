...
@Component({
  selector: 'app-confirm-order-swp',
  standalone: true,
  ...
  templateUrl: './confirm-order-swp.component.html',
  styleUrl: './confirm-order-swp.component.scss',
  providers: [
...
export class ConfirmOrderSwpComponent implements OnInit, OnChanges {
      ...
      this.transactionService.GetClientTransactionDetails(this.clientTransactionId).subscribe((result) => {
        if (result.Status == true) {
          ...
          if (swpPortfolioItem != null) {
            ...
            this.sellSWPAllocation = swpPortfolioItem.Allocations;
            this.sipTransactionType = swpPortfolioItem.SIPTransactionType;
            if (this.sipTransactionType == 'M') {
              this.authTabNumber = 2;
            }
          ...
          if (this.clientTransactionDetails.TransactionPlanCode == 'ASWP') {
  ...
  getSellPortfolioAllocation() {
    this.transactionService.GetClientTransactionAllocationSell(this.clientTransactionId).subscribe((result) => {
      if (result.Status == true) {
        ...
        this.sellASWPAllocation = result.Data;

        for (let a = 0; a < this.sellASWPAllocation.length; a++) {
          var allocationItem = this.sellASWPAllocation[a];
    ...
    this.appUserService.ValidateTransactionClient(userInputData).subscribe((userResult) => {
      if (userResult.Status == true) {
        ...
        var inputData = {
          ...
          ReturnUrlParam: 'confirm-order-swp/' + this.clientId + '/' + this.clientProfileId + '/' + this.clientTransactionId + '/' + this.appCryptoService.ParamEncrypt(this.mode) + '/' + this.appCryptoService.ParamEncrypt(this.ts)
        }

        this.transactionService.SaveClientSellSWPTransactionApproval(inputData).subscribe((result) => {
          // console.log(result);
          if (result.Status == true) {
