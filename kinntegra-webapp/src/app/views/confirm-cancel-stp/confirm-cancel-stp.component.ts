...
@Component({
  selector: 'app-confirm-cancel-stp',
  standalone: true,
  ...
  templateUrl: './confirm-cancel-stp.component.html',
  styleUrl: './confirm-cancel-stp.component.scss',
  providers: [
...
export class ConfirmCancelStpComponent implements OnInit, OnChanges {
    ...
    this.appUserService.ValidateTransactionClient(userInputData).subscribe((userResult) => {
      if (userResult.Status == true) {
        var inputData = {
          ...
          ReturnUrlParam: 'confirm-cancel-stp/' + this.clientId + '/' + this.clientProfileId + '/' + this.clientTransactionId + '/' + this.appCryptoService.ParamEncrypt(this.mode) + '/' + this.appCryptoService.ParamEncrypt(this.ts)
        }

        this.transactionService.SaveClientSTPCancelTransactionApproval(inputData).subscribe((result) => {
          // console.log(result);
          if (result.Status == true) {
