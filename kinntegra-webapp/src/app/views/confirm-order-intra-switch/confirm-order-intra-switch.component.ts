...
@Component({
  selector: 'app-confirm-order-intra-switch',
  standalone: true,
  ...
  templateUrl: './confirm-order-intra-switch.component.html',
  styleUrl: './confirm-order-intra-switch.component.scss',
  providers: [
...
export class ConfirmOrderIntraSwitchComponent implements OnInit, OnChanges {
    ...
    this.appUserService.ValidateTransactionClient(userInputData).subscribe((userResult) => {
      if (userResult.Status == true) {
        var inputData = {
          ...
          ReturnUrlParam: 'confirm-order-intra-switch/' + this.clientId + '/' + this.clientProfileId + '/' + this.clientTransactionId + '/' + this.appCryptoService.ParamEncrypt(this.mode) + '/' + this.appCryptoService.ParamEncrypt(this.ts)
        }

        this.transactionService.SaveClientIntraSwitchTransactionApproval(inputData).subscribe((result) => {
          // console.log(result);
          if (result.Status == true) {
