...
@Component({
  selector: 'app-scheme-master-upload',
  standalone: true,
  ...
  templateUrl: './scheme-master-upload.component.html',
  styleUrl: './scheme-master-upload.component.scss',
  providers: [BseImportService]
})
export class SchemeMasterUploadComponent {
  ...
  onSaveExcel() {
    ...
    this.bseImportService.UploadSchemeExcel(inputData).subscribe({
      next: (result) => {
        if (result.Status) {
          this.LastModifiedDate = result.MaxModifiedDate;
          // console.log(this.LastModifiedDate)
