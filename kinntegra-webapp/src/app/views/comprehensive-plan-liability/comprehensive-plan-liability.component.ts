...
@Component({
  selector: 'app-comprehensive-plan-liability',
  standalone: true,
  ...
  templateUrl: './comprehensive-plan-liability.component.html',
  styleUrl: './comprehensive-plan-liability.component.scss',
  providers: [ClientService, AppCryptoService, ComprehensivePlanService]
})
export class ComprehensivePlanLiabilityComponent {
  ...
  getClientKycDetails() {
    ...
    this.comprehensivePlanService.GetComprehensivePlanLiability(this.clientId).subscribe((cpresult) => {
      if (cpresult.Status == true) {
        var data = cpresult.Data;
        

        for (let i = 0; i < data.length; i++) {
          var selectedCategoryItem = this.IncomeCategories.find((x: any) => x.CategoryName.toLowerCase() === data[i].LiabilityCategory.toLowerCase());
          
          var selectedCategoryItem = this.SelectedCategories.find((x: { CategoryName: string; }) => x.CategoryName.toLowerCase() === data[i].LiabilityCategory.toLowerCase());
          
          if (selectedCategoryItem == null || selectedCategoryItem == undefined) {
            var categoryItem = this.IncomeCategories.find((x: { CategoryName: string; }) => x.CategoryName.toLowerCase() === data[i].LiabilityCategory.toLowerCase());
            

            var k = this.IncomeCategories.findIndex((x: { CategoryName: string; }) => x.CategoryName.toLowerCase() === data[i].LiabilityCategory.toLowerCase());
          ...
          if (selectedCategoryItem != null && selectedCategoryItem != undefined) {
            var incomeDataItem = {
              Id: data[i].Id,
              EMI: data[i].EMI,
              RemainingInstallment: data[i].RemainingInstallment,
  ...
  onSave() {
    ...
    for (let i = 0; i < this.SelectedCategories.length; i++) {
      for (let j = 0; j < this.SelectedCategories[i].IncomeData.length; j++) {
        ...
        var incomeItem = {
          LiabilityCategory: this.SelectedCategories[i].CategoryName,
          EMI: incomeDataItem.EMI,
          RemainingInstallment: incomeDataItem.RemainingInstallment,
          InterestRate: incomeDataItem.InterestRate,
    ...
    let inputData = {
      ...
      ClientLiabilityDetails: JSON.stringify(ClientIncomeData),

    };
 

    this.comprehensivePlanService.SaveComprehensivePlanLiability(inputData).subscribe(
      (result) => {
