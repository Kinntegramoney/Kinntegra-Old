...
import { ChangeDetectorRef, Component, ElementRef, OnChanges, OnInit, QueryList, Renderer2, SimpleChanges, ViewChild, ViewChildren } from '@angular/core';
...
@Component({
  selector: 'app-transaction-intra-switch-portfolio',
  standalone: true,
  imports: [TransactionIntraSwitchLeftbarTemplateComponent, FormsModule, CommonModule, NgSelectModule, NgbModule, NgbDropdownModule, HeaderRightTemplateComponent, HttpClientModule, NgxDatatableModule, IndianCurrencyNumberPipe],
  templateUrl: './transaction-intra-switch-portfolio.component.html',
  styleUrl: './transaction-intra-switch-portfolio.component.scss',
  providers: [
    TransactionService,
  ]
})
export class TransactionIntraSwitchPortfolioComponent implements OnInit, OnChanges {
  ...
  getClientTransactionById() {
    this.transactionService.GetClientTransactionDetails(this.clientTransactionId).subscribe((result) => {
      if (result.Status == true) {
        ...
        var firstHolder = result.Data.ClientAccount.AccountHolders.find((x: { SerialNumber: number; }) => x.SerialNumber == 1);
  ...
  onProceed() {
    ...
    this.transactionService.SaveIntraSwitchPortfolioAllocation(inputData).subscribe(
      (result) => {
        if (result.Status == true) {
          this.isBusy = false;
          this.router.navigate(['transaction/intra-switch/allocation/' + this.clientTransactionId]);
        }
        else {
          this.isBusy = false;
          this.appErrors = [];
