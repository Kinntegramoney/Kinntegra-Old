...
@Component({
  selector: 'app-transaction-allocation-stp-cancel',
  standalone: true,
  ...
  templateUrl: './transaction-allocation-stp-cancel.component.html',
  styleUrl: './transaction-allocation-stp-cancel.component.scss',
  providers: [
...
export class TransactionAllocationStpCancelComponent implements OnInit, OnChanges {
    ...
    this.transactionService.SaveSTPCancelAllocation(inputData).subscribe(
      (result) => {
        if (result.Status == true) {
          ...
          var userMessage = "Trade Initiated for Admin approval! Do you want to make another transaction?";
