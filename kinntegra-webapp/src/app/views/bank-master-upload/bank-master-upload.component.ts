...
@Component({
  selector: 'app-bank-master-upload',
  standalone: true,
  ...
  templateUrl: './bank-master-upload.component.html',
  styleUrl: './bank-master-upload.component.scss',
  providers: [BankService]
})
export class BankMasterUploadComponent {
  ...
  constructor(
    ...
    private bankMasterUploadService: BankService,
  ...
  uploadDocumentExcel(input: any) {
    this.bankMasterUploadService.UploadExcel(input).subscribe({
      next: (result) => {
        if (result.Status) {
          this.LastModifiedDate = result.MaxModifiedDate;
          // console.log(this.LastModifiedDate)
          this.alertBoxMsg(result.Message, result.Status, '/bank-upload');
        } else {
