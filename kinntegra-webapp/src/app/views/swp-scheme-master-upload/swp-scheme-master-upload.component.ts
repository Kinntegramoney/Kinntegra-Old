...
@Component({
  selector: 'app-swp-scheme-master-upload',
  standalone: true,
  ...
  templateUrl: './swp-scheme-master-upload.component.html',
  styleUrl: './swp-scheme-master-upload.component.scss',
  providers: [BseImportService]
})

export class SwpSchemeMasterUploadComponent {
  ...
  onSaveExcel() {
    ...
    this.bseImportService.UploadSWPSchemeExcel(inputData).subscribe({
      next: (result) => {
        if (result.Status) {
          this.LastModifiedDate = result.MaxModifiedDate;
          // console.log(this.LastModifiedDate)
          this.alertBoxMsg(result.Message, result.Status, '/swp-scheme-upload');
        } else {
          this.alertBoxMsg(result.Message, result.Status, null);
        }
      },
