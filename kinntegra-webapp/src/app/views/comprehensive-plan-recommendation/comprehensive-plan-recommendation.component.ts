...
import { NgbModule, NgbDropdownModule, NgbDateAdapter, NgbModalOptions, NgbDropdown, NgbNavModule } from '@ng-bootstrap/ng-bootstrap';
...
@Component({
  selector: 'app-comprehensive-plan-recommendation',
  standalone: true,
  imports: [FormsModule,CommonModule,NgSelectModule,NgbModule,NgbDropdownModule,HeaderRightTemplateComponent,ClientLeftbarTemplateComponent, NgbNavModule, RouterLink, NgxDatatableModule, NgbDropdown, HttpClientModule],
  templateUrl: './comprehensive-plan-recommendation.component.html',
  styleUrl: './comprehensive-plan-recommendation.component.scss'
})
export class ComprehensivePlanRecommendationComponent {
  ...
  onNext() {
    if (this.mode == null) {
      this.router.navigate(['comprehensive-plan-client-kyc-details/' + this.clientId]);
    }
    else {
      this.router.navigate(['comprehensive-plan-client-kyc-details/' + this.clientId + '/' + this.appCryptoService.ParamEncrypt(this.mode)]);
    }
  }
}
