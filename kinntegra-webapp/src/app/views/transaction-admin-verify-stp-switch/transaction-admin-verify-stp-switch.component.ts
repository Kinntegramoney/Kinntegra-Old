...
@Component({
  selector: 'app-transaction-admin-verify-stp-switch',
  standalone: true,
  ...
  templateUrl: './transaction-admin-verify-stp-switch.component.html',
  styleUrl: './transaction-admin-verify-stp-switch.component.scss',
  providers: [
...
export class TransactionAdminVerifyStpSwitchComponent implements OnInit, OnChanges {
  ...
  onApprovedClicked() {
    ...
    this.transactionService.SaveAdminStpSwitchPortfolioAllocation(inputData).subscribe(
      (result) => {
        if (result.Status == true) {
