...
@Component({
  selector: 'app-benchmarkdata-upload',
  standalone: true,
  imports: [HeaderRightTemplateComponent, HttpClientModule, NgbNavModule, NgxDatatableModule, CommonModule, NgbDropdownModule],
  templateUrl: './benchmarkdata-upload.component.html',
  styleUrl: './benchmarkdata-upload.component.scss',
  providers: [BenchmarkdataService]
})
export class BenchmarkdataUploadComponent {
  ...
  uploadDocumentExcel(input: any) {
    this.benchmarkdataService.UploadBenchmarkData(input).subscribe({
      next: (result) => {
        if (result.Status) {
          this.alertBoxMsg(result.Message, result.Status, '/benchmarkdata-upload');
        } else {
