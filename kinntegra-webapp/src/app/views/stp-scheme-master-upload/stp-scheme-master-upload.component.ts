...
@Component({
  selector: 'app-stp-scheme-master-upload',
  standalone: true,
  ...
  templateUrl: './stp-scheme-master-upload.component.html',
  styleUrl: './stp-scheme-master-upload.component.scss',
  providers: [BseImportService]
})

export class StpSchemeMasterUploadComponent {
  ...
  onSaveExcel() {
    ...
    this.bseImportService.UploadSTPSchemeExcel(inputData).subscribe({
      next: (result) => {
        if (result.Status) {
          this.LastModifiedDate = result.MaxModifiedDate;
          // console.log(this.LastModifiedDate)
          this.alertBoxMsg(result.Message, result.Status, '/stp-scheme-upload');
        } else {
          this.alertBoxMsg(result.Message, result.Status, null);
        }
      },
